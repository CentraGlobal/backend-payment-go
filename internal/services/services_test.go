...
func NewStubPaymentGatewayService() *StubPaymentGatewayService {
	return &StubPaymentGatewayService{Gateways: map[string]*domain.PaymentGateway{}}
}

func (s *StubPaymentGatewayService) GetActiveDefault(_ context.Context, orgID, hotelID string) (*domain.PaymentGateway, error) {
	for _, gw := range s.Gateways {
		if gw.OrgID == orgID && gw.HotelID == hotelID &&
			gw.Status == domain.GatewayStatusActive && gw.IsDefault {
			return gw, nil
		}
	}
	return nil, nil
...
func (s *StubPaymentGatewayService) GetByID(_ context.Context, id string) (*domain.PaymentGateway, error) {
	gw, ok := s.Gateways[id]
	if !ok {
		return nil, nil
	}
...
type StubCardTokenService struct {
	tokens map[string]*domain.CardToken
	nextID int
}

func NewStubCardTokenService() *StubCardTokenService {
	return &StubCardTokenService{tokens: map[string]*domain.CardToken{}}
}

func (s *StubCardTokenService) Create(_ context.Context, token *domain.CardToken) (*domain.CardToken, error) {
	s.nextID++
...
func (s *StubCardTokenService) GetByID(_ context.Context, id string) (*domain.CardToken, error) {
	t, ok := s.tokens[id]
	if !ok {
		return nil, nil
	}
...
func (s *StubCardTokenService) GetByVaultToken(_ context.Context, gatewayID, vaultToken string) (*domain.CardToken, error) {
...
func (s *StubCardTokenService) UpdateStatus(_ context.Context, id string, status domain.CardTokenStatus) error {
...
type StubTransactionService struct {
	...
	nextID       int
	LastUpdate   *domain.TransactionStatusUpdate
}

func NewStubTransactionService() *StubTransactionService {
	return &StubTransactionService{transactions: map[string]*domain.Transaction{}}
}

func (s *StubTransactionService) CreatePending(_ context.Context, tx *domain.Transaction) (*domain.Transaction, error) {
	s.nextID++
...
func (s *StubTransactionService) UpdateStatus(_ context.Context, id string, u domain.TransactionStatusUpdate) error {
	s.LastUpdate = &u
...
func (s *StubTransactionService) GetByID(_ context.Context, id string) (*domain.Transaction, error) {
...
func TestStubPaymentGatewayService_GetActiveDefault(t *testing.T) {
	...
	svc.Gateways["gw_1"] = &domain.PaymentGateway{
		ID:        "gw_1",
		OrgID:     "org_1",
		HotelID:   "hotel_1",
		Provider:  domain.GatewayProviderStripe,
		Mode:      domain.GatewayModeLive,
		Status:    domain.GatewayStatusActive,
		IsDefault: true,
	}
...
func TestStubPaymentGatewayService_GetByID(t *testing.T) {
	svc := NewStubPaymentGatewayService()
	svc.Gateways["gw_42"] = &domain.PaymentGateway{ID: "gw_42", OrgID: "org_1", HotelID: "hotel_1"}
...
func TestStubCardTokenService_CreateAndGet(t *testing.T) {
	...
	token := &domain.CardToken{
		...
		VaultToken:    "tok_xyz",
		TokenScope:    domain.TokenScopeSingleUse,
...
func TestStubCardTokenService_GetByVaultToken(t *testing.T) {
	svc := NewStubCardTokenService()
	svc.Create(context.Background(), &domain.CardToken{
		OrgID: "org_1", HotelID: "hotel_1", GatewayID: "gw_1",
		VaultProvider: "vaultera", VaultToken: "tok_abc",
		TokenScope: domain.TokenScopeSingleUse, Status: domain.TokenStatusActive,
	})
...
func TestStubCardTokenService_UpdateStatus(t *testing.T) {
	svc := NewStubCardTokenService()
	token, _ := svc.Create(context.Background(), &domain.CardToken{
		OrgID: "org_1", HotelID: "hotel_1", GatewayID: "gw_1",
		VaultProvider: "vaultera", VaultToken: "tok_def",
		TokenScope: domain.TokenScopeSingleUse, Status: domain.TokenStatusActive,
	})

	if err := svc.UpdateStatus(context.Background(), token.ID, domain.TokenStatusConsumed); err != nil {
		t.Fatalf("UpdateStatus: %v", err)
	}

	fetched, _ := svc.GetByID(context.Background(), token.ID)
	if fetched.Status != domain.TokenStatusConsumed {
...
func TestStubTransactionService_CreatePending(t *testing.T) {
	...
	if created.Status != domain.TxStatusPending {
...
func TestStubTransactionService_UpdateStatus_Succeeded(t *testing.T) {
	svc := NewStubTransactionService()
	tx, _ := svc.CreatePending(context.Background(), &domain.Transaction{
		OrgID: "org_1", HotelID: "hotel_1", GatewayID: "gw_1",
		Operation: domain.TxOpCharge, Amount: 1000, Currency: "EUR",
	})
	...
	if fetched.Status != domain.TxStatusSucceeded {
...
func TestStubTransactionService_UpdateStatus_Failed(t *testing.T) {
	...
	tx, _ := svc.CreatePending(context.Background(), &domain.Transaction{
		OrgID: "org_1", HotelID: "hotel_1", GatewayID: "gw_1",
		Operation: domain.TxOpCharge, Amount: 2000, Currency: "GBP",
	})
	...
	if svc.LastUpdate.Status != domain.TxStatusFailed {
...
func TestPostgresCardTokenService_NilPool(t *testing.T) {
	...
	if _, err := svc.Create(context.Background(), &domain.CardToken{}); err == nil {
		t.Error("expected error with nil pool")
	}
	if _, err := svc.GetByID(context.Background(), "id"); err == nil {
		t.Error("expected error with nil pool")
	...
	if err := svc.UpdateStatus(context.Background(), "id", domain.TokenStatusRevoked); err == nil {
		t.Error("expected error with nil pool")
	}
}
...
func TestPostgresTransactionService_NilPool(t *testing.T) {
	...
	if _, err := svc.CreatePending(context.Background(), &domain.Transaction{}); err == nil {
		t.Error("expected error with nil pool")
	}
	if err := svc.UpdateStatus(context.Background(), "id", domain.TransactionStatusUpdate{}); err == nil {
		t.Error("expected error with nil pool")
	}
	if _, err := svc.GetByID(context.Background(), "id"); err == nil {
		t.Error("expected error with nil pool")
